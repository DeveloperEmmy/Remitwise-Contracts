...
fn test_propose_split_config_change() {
    ...
    let tx_id = client
        .propose_split_config_change(&owner, &spending, &savings, &bills, &insurance)
        .unwrap();
    assert!(tx_id > 0);
...
fn test_pending_transactions_pagination_and_auth() {
    ...
    env.mock_all_auths();
    client
        .propose_split_config_change(&member1, &10, &40, &30, &20)
        .unwrap();
    env.mock_all_auths();
    client
        .propose_split_config_change(&member2, &11, &39, &30, &20)
        .unwrap();
    env.mock_all_auths();
    client
        .propose_split_config_change(&member1, &12, &38, &30, &20)
        .unwrap();
    env.mock_all_auths();
    client
        .propose_split_config_change(&member2, &13, &37, &30, &20)
        .unwrap();
    env.mock_all_auths();
    client
        .propose_split_config_change(&member1, &14, &36, &30, &20)
        .unwrap();
