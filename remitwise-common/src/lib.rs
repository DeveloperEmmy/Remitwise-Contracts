...
// ---------------------------------------------------------------------------
// Event payload version validation (#1180)
// ---------------------------------------------------------------------------

/// Error returned when an event payload version does not match the expected version.
#[contracterror]
#[derive(Copy, Clone, Debug, Eq, PartialEq, PartialOrd, Ord)]
#[repr(u32)]
pub enum EventVersionError {
    /// The event payload version does not match the current contract version.
    /// This indicates the event was produced by a different contract version
    /// and may have an incompatible schema.
    VersionMismatch = 1,
}

/// Guards that the event payload version matches the expected version.
///
/// This is a defence-in-depth check: if the contract is upgraded to a new event
/// schema version, old-format events referencing the prior version should be
/// rejected rather than interpreted under the new schema (which could cause
/// data corruption, storage-key collisions, or indexer deserialization errors).
///
/// # Threat model
/// Without this guard, an attacker (or a stale off-chain relayer) could submit
/// an event payload bearing a version number different from the current
/// `CONTRACT_VERSION`. If the contract logic interprets the payload under the
/// assumption that its version matches the current schema, the mismatch could
/// lead to:
/// - **Deserialization confusion**: fields shifted in offset or meaning between
///   versions could be read as the wrong type or value.
/// - **Storage corruption**: a payload deserialized with mismatched field
///   layout could overwrite unrelated storage keys.
/// - **Indexer/replay divergence**: off-chain consumers that key events by
///   version may silently drop or mis-classify events that carry an unexpected
///   version tag.
///
/// The check is a single `u32` comparison with `Env::cost_estimate()` well
/// below any meaningful entry-point overhead, making it suitable even for
/// hot-path event ingestion.
///
/// # Errors
/// Returns [`EventVersionError::VersionMismatch`] when `payload_ver != current`.
pub fn require_matching_event_version(payload_ver: u32, current: u32) -> Result<(), EventVersionError> {
    if payload_ver != current {
        Err(EventVersionError::VersionMismatch)
    } else {
        Ok(())
    }
}
...
mod event_version_tests {
    ...
    fn accepts_matching_version() {
        assert_eq!(require_matching_event_version(1, 1), Ok(()));
    }

    #[test]
    fn accepts_zero_version_when_matching() {
        assert_eq!(require_matching_event_version(0, 0), Ok(()));
    }

    #[test]
    fn accepts_large_matching_version() {
        assert_eq!(require_matching_event_version(u32::MAX, u32::MAX), Ok(()));
    }

    #[test]
    fn rejects_version_mismatch_payload_lower() {
        assert_eq!(
            require_matching_event_version(0, 1),
            Err(EventVersionError::VersionMismatch)
        );
    }

    #[test]
    fn rejects_version_mismatch_payload_higher() {
        assert_eq!(
            require_matching_event_version(2, 1),
            Err(EventVersionError::VersionMismatch)
        );
    }

    #[test]
    fn rejects_version_mismatch_large_values() {
        assert_eq!(
            require_matching_event_version(u32::MAX - 1, u32::MAX),
            Err(EventVersionError::VersionMismatch)
        );
    }
}
